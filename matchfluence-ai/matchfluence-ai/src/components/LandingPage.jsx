  ...
  return (
    <section className="mx-auto flex min-h-[90vh] w-full max-w-6xl flex-col justify-center px-4 py-10 sm:px-6 lg:px-8">
      <div className="grid gap-8 lg:grid-cols-[1.1fr_.9fr] lg:items-center">
        <div>
          ...
          <div className="mt-6 inline-flex rounded-full border border-white/10 bg-white/5 px-3 py-1 text-sm text-slate-300">
            Yapay zeka destekli mikro-influencer eşleştirme MVP'si
          </div>
          <h1 className="mt-5 text-4xl font-black tracking-tight text-white sm:text-6xl">
            Lokal işletmeler için doğru influencer'ı saniyeler içinde bul.
          </h1>
          <p className="mt-5 max-w-2xl text-base leading-7 text-slate-300 sm:text-lg">
            Matchfluence AI, kampanya hedefinizi alır, mock influencer verisini açıklanabilir bir skor motoruyla analiz eder ve Tinder benzeri mobil kart deneyimiyle en uygun mikro-influencer adaylarını gösterir.
          </p>
          ...
          <div className="relative rounded-[1.5rem] bg-slate-950/60 p-5">
            <div className="flex items-center justify-between">
              <div>
                ...
                <h2 className="text-2xl font-black text-white">Derya Koç</h2>
              </div>
            ...
            <div className="mt-5 space-y-3">
              {['Niş uyumu yüksek', 'Kadıköy lokasyon avantajı', '8.2% etkileşim oranı', 'Bütçeye uygun mikro-influencer'].map((item) => (
                <div key={item} className="flex items-center gap-3 rounded-2xl border border-white/10 bg-white/5 p-3 text-sm text-slate-200">
                  <span className="flex h-7 w-7 items-center justify-center rounded-full bg-indigo-400/20 text-indigo-100">✓</span>
                  {item}
                </div>
              ))}
            </div>
      ...
      <div id="how-it-works" className="mt-12 grid gap-4 sm:grid-cols-3">
        {[
          ['1', 'Kampanya bilgisi', 'İşletme, hedef kitle, bütçe ve niş bilgilerini girer.'],
          ['2', 'Açıklanabilir skor', 'Niş, lokasyon, etkileşim, kitle, bütçe ve deneyim birlikte puanlanır.'],
          ['3', 'Swipe demo', 'Kartları sağa/sola kaydırır, eşleşme nedenini görür.'],
        ].map(([step, title, text]) => (
          <div key={step} className="rounded-3xl border border-white/10 bg-white/[0.04] p-5">
            <div className="mb-4 flex h-10 w-10 items-center justify-center rounded-2xl bg-white text-lg font-black text-slate-950">{step}</div>
