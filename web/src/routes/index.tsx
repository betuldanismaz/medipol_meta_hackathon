...
export const Route = createFileRoute("/")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Tinder tarzı swipe ve AI ile yerel işletmeleri uygun influencer'larla buluşturuyoruz." },
      { property: "og:title", content: "InfluMatch" },
      { property: "og:description", content: "Kafeler, butikler ve influencer'lar için akıllı eşleşme." },
    ],
  }),
  component: Home,
});
    ...
    <SiteShell>
          ...
          <div>
            ...
            <div className="mt-8 flex flex-wrap gap-3">
              <Button asChild size="lg" className="bg-gradient-brand text-primary-foreground hover:opacity-90">
                <Link to="/auth/register">Hemen Başla <ArrowRight className="ml-2 w-4 h-4" /></Link>
              </Button>
              <Button asChild size="lg" variant="outline">
                <Link to="/discover">Keşfetmeye Başla</Link>
              </Button>
            </div>
      ...
      <section className="mx-auto max-w-7xl px-4 sm:px-6 pb-20">
        <div className="flex items-end justify-between mb-8">
          ...
          <Button asChild variant="outline"><Link to="/listings">Tümünü gör</Link></Button>
        </div>
        <div className="grid md:grid-cols-3 gap-6">
          {listings.slice(0, 3).map((l) => (
            <Link key={l.id} to="/listings/$id" params={{ id: l.id }} className="group rounded-2xl border border-border overflow-hidden bg-card hover:shadow-[var(--shadow-glow)] transition">
      ...
      <section className="mx-auto max-w-7xl px-4 sm:px-6 pb-24">
        <div className="rounded-3xl bg-gradient-brand p-10 md:p-16 text-center text-primary-foreground relative overflow-hidden">
          ...
          <div className="relative mt-8 flex justify-center gap-3 flex-wrap">
            <Button asChild size="lg" variant="secondary"><Link to="/auth/register">Hesap Aç</Link></Button>
            <Button asChild size="lg" variant="outline" className="bg-transparent border-white/30 text-white hover:bg-white/10"><Link to="/how-it-works">Daha Fazla</Link></Button>
          </div>
        </div>
      </section>
    </SiteShell>
