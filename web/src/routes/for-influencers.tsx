...
export const Route = createFileRoute("/for-influencers")({
  head: () => ({
    meta: [
      { title: "Influencer'lar İçin — InfluMatch" },
      { name: "description", content: "Geliriniz için doğru markalarla eşleşin. AI seviyelendirme ile uygun fırsatlar." },
    ],
  }),
  component: ForInfluencers,
});

function ForInfluencers() {
  return (
    <SiteShell>
      <section className="mx-auto max-w-7xl px-4 sm:px-6 py-20 grid lg:grid-cols-2 gap-12 items-center">
        <div>
          ...
          <div className="mt-8 flex gap-3">
            <Button asChild size="lg" className="bg-gradient-brand text-primary-foreground"><Link to="/auth/register">Influencer Olarak Katıl</Link></Button>
            <Button asChild size="lg" variant="outline"><Link to="/listings">İlanları Gör</Link></Button>
          </div>
        </div>
