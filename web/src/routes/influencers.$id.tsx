...
import { MapPin, Sparkles, Users, ArrowLeft, Star } from "lucide-react";
import { influencers } from "@/lib/mock-data";
import { toast } from "sonner";

export const Route = createFileRoute("/influencers/$id")({
  head: ({ params }) => {
    const i = influencers.find((x) => x.id === params.id);
    return { meta: [{ title: i ? `${i.name} — Influencer` : "Profil" }] };
  },
  loader: ({ params }) => {
    const i = influencers.find((x) => x.id === params.id);
    if (!i) throw notFound();
  ...
  notFoundComponent: () => (
    <SiteShell>
      <div className="mx-auto max-w-2xl px-4 py-24 text-center">
        ...
        <Button asChild className="mt-6"><Link to="/influencers">Profillere dön</Link></Button>
      </div>
    </SiteShell>
  ),
  ...
  return (
    <SiteShell>
      <section className="mx-auto max-w-5xl px-4 sm:px-6 py-10">
        <Link to="/influencers" className="inline-flex items-center text-sm text-muted-foreground hover:text-foreground mb-6">
        ...
        <div className="grid md:grid-cols-3 gap-8">
          ...
          <div className="md:col-span-2">
            ...
            <div className="flex flex-wrap gap-2">
              {i.niches.map((n: string) => <Badge key={n} variant="secondary">{n}</Badge>)}
            </div>

            <h3 className="font-semibold mt-6 mb-2">Son işbirlikleri</h3>
            <ul className="space-y-2">
              {i.recentWork.map((w: string) => <li key={w} className="text-sm text-muted-foreground">• {w}</li>)}
            </ul>

            <div className="mt-8 flex gap-3">
              <Button onClick={() => toast.success("İşbirliği teklifi gönderildi!")} className="bg-gradient-brand text-primary-foreground hover:opacity-90">
                Teklif Gönder
              </Button>
