...
export default function InfluencerSwipeCards({
  ...
  const progress = useMemo(
    () => Math.round((index / Math.max(rankedInfluencers.length, 1)) * 100),
    [index, rankedInfluencers.length],
  );
  ...
  const moveNext = (direction: "accept" | "reject") => {
    ...
    setSwipeHint(direction === "accept" ? "Saga kaydirildi" : "Sola kaydirildi");
    ...
    return (
      <section className="mx-auto flex min-h-screen w-full max-w-3xl items-center px-4 py-8">
        <div className="card-gradient w-full rounded-[2rem] p-6 text-center">
          ...
          <h1 className="mt-5 text-3xl font-black text-white">Tum kartlar incelendi</h1>
          <p className="mt-3 text-slate-300">
            Sectigin influencer'lar match ekraninda listelendi. Yeni demo icin basa donebilirsin.
          </p>
          <button
            onClick={onRestart}
            className="mt-6 rounded-2xl bg-white px-6 py-3 font-bold text-slate-950"
          >
    ...
    <section className="mx-auto flex min-h-screen w-full max-w-6xl flex-col px-4 py-6 sm:px-6 lg:px-8">
      <div className="mb-5 flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
        <div>
          ...
          <h1 className="mt-3 text-3xl font-black text-white">Swipe eslestirme</h1>
          <p className="mt-2 text-sm text-slate-400">
            {campaign.businessName} icin skorlanmis influencer kartlari. Mobilde saga veya sola kaydirabilirsin.
          </p>
        </div>
        <div className="rounded-2xl border border-white/10 bg-white/5 px-4 py-3 text-sm text-slate-300">
          {index + 1}/{rankedInfluencers.length} kart | %{progress} tamamlandi
        </div>
      </div>
      ...
      <div className="grid flex-1 gap-6 lg:grid-cols-[minmax(0,1fr)_330px] lg:items-center">
          ...
          >
            ...
            <div className="relative">
              <div className="flex items-start justify-between gap-4">
                <div>
                  <p className="text-sm text-slate-400">#{index + 1} oneri</p>
              ...
              <div className="mt-5 rounded-3xl bg-white/[0.06] p-4">
                <div className="flex items-end justify-between">
                  ...
                  <span className="text-5xl font-black text-white">{current.matchScore.score}</span>
                </div>
                <div className="mt-3 h-3 overflow-hidden rounded-full bg-white/10">
                  <div
                    className="h-full rounded-full bg-gradient-to-r from-emerald-300 via-cyan-200 to-indigo-300"
              ...
              <div className="mt-5 grid grid-cols-3 gap-2 text-center">
                ...
                <Metric label="Engagement" value={`%${current.engagementRate}`} />
                <Metric label="Fiyat" value={`${current.price.toLocaleString("tr-TR")} TL`} />
              </div>

              <p className="mt-5 rounded-3xl border border-white/10 bg-slate-950/50 p-4 text-sm leading-6 text-slate-300">
                {current.bio}
              </p>
              {swipeHint ? (
                <p className="mt-3 text-center text-sm font-bold text-emerald-200">{swipeHint}</p>
              ) : null}

              <div className="safe-area mt-6 grid grid-cols-2 gap-3">
                <button
                  onClick={() => moveNext("reject")}
                  className="rounded-2xl border border-rose-300/20 bg-rose-400/10 px-5 py-4 font-black text-rose-100 transition hover:bg-rose-400/20"
                >
                  Pas gec
                </button>
                <button
                  onClick={() => moveNext("accept")}
                  className="rounded-2xl border border-emerald-300/20 bg-emerald-300 px-5 py-4 font-black text-emerald-950 transition hover:bg-emerald-200"
                >
                  Sec
                </button>
              </div>
        ...
        <aside className="card-gradient rounded-[2rem] p-5">
          <h3 className="text-xl font-black text-white">Skor kirilimi</h3>
          <p className="mt-2 text-sm text-slate-400">
            Sistem, takipci sayisini tek basina kriter yapmaz; mikro-influencer kalitesini aciklanabilir uyum sinyalleriyle olcer.
          </p>
          <div className="mt-5">
          ...
          <div className="mt-5 rounded-3xl bg-white/[0.06] p-4">
            ...
            <ul className="mt-2 space-y-2 text-sm leading-6 text-slate-200">
              {current.matchScore.reasons.map((reason) => (
