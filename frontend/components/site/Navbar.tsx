    ...
    <header className="sticky top-0 z-40 backdrop-blur-xl bg-background/70 border-b border-border/60">
      <div className="mx-auto max-w-7xl px-4 sm:px-6 h-16 flex items-center justify-between">
        ...
        <div className="hidden md:flex items-center gap-2">
          <Button asChild variant="ghost" size="sm"><Link href="/auth/login">Giriş</Link></Button>
      ...
      {open && (
        <div className="md:hidden border-t border-border/60 bg-background">
          <div className="px-4 py-3 flex flex-col gap-1">
            {nav.map((n) => (
              <Link key={n.to} href={n.to} className="py-2 text-sm" onClick={() => setOpen(false)}>{n.label}</Link>
            ))}
            <div className="flex gap-2 pt-2">
              <Button asChild variant="outline" size="sm" className="flex-1"><Link href="/auth/login">Giriş</Link></Button>
              <Button asChild size="sm" className="flex-1 bg-gradient-brand text-primary-foreground"><Link href="/auth/register">Üye Ol</Link></Button>
            </div>
          </div>
        </div>
      )}
