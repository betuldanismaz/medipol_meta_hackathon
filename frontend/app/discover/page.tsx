...
import { influencers, type Influencer } from "@/lib/mock-data";
...
export default function DiscoverPage() {
  ...
  const swipe = (dir: 1 | -1) => {
    setDirection(dir);
    if (dir === 1 && current) toast.success(`${current.name} ile beğeni gönderildi!`);
    setTimeout(() => {
      setIndex((i) => i + 1);
      setDirection(0);
    }, 250);
    ...
    <SiteShell>
      <section className="mx-auto max-w-2xl px-4 sm:px-6 py-10">
        ...
        <div className="mt-8 text-center text-sm text-muted-foreground">
          <Link href="/listings" className="underline">Bunun yerine ilanlara göz at</Link>
        </div>
      </section>
    </SiteShell>
  );
...
function Card({ inf, stacked = false }: { inf: Influencer; stacked?: boolean }) {
  return (
    <div
